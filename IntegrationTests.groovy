...
pipeline {
    ...
    stages {
                    ...
                    script {
                        mvnCommandLine =
                            """
                                ...
                                -Dserver.https.port=$payara_config.ssl_port -DjacocoPort=$payara_config.jacoco_port \
                            """
                    }
                }
            }
        ...
        stage('Maven Deploy Javadoc and Snapshots') {
            ...
            steps {
                sh "mvn -B -C -fae deploy -Dmaven.install.skip=true -Dcheckstyle.skip=true -DskipTests -DskipITs -P$profiles"
            }
        }
        stage('Maven Deploy documentation') {
            ...
            steps {
                ...
                rsync -aH --delete-after ${env.WORKSPACE}/flowlogix-jee/target/reports/apidocs/ \
                    ${websiteHost()}:/var/flowlogix/html/javadoc/jee-apidocs/
                rsync -aH --delete-after ${env.WORKSPACE}/flowlogix-datamodel/target/reports/apidocs/ \
